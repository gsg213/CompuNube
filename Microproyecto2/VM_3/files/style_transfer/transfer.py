...
class Transfer(Resource):
    ...
    def post(self):
        ...
        content = self.load_image(content_file).to(self.device)
        # Update content image
        plt.imshow(self.im_convert(content))
        os.remove('static/content.jpg')
        plt.savefig('static/content.jpg')
        # Resize style to match content, makes code easier
        style = self.load_image(style_file, shape=content.shape[-2:]).to(self.device)
        # Update style image
        plt.imshow(self.im_convert(style))
        os.remove('static/style.jpg')
        plt.savefig('static/style.jpg')
